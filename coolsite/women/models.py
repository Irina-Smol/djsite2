...
class Women(models.Model): #поле id автоматически прописано в этом классе
    ...
    content = models.TextField(blank=True, verbose_name="Текст статьи") #blank=True - поле может быть пустым
...
#для работы с ORM, используется консоль shell внутри фреймворка
    ...
    class Meta:        # вложенный класс для более точной работы с admin-панелью
        verbose_name = 'Известные женщины'    # замена названия в админке
        verbose_name_plural = 'Известные женщины' # чтобы убрать s от множественного числа
